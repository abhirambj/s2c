...
export const useInfiniteCanvas = () => {
	...
	const shapesList: Shape[] = entityState.ids
	...
	const onPointerDown: React.PointerEventHandler<HTMLDivElement> = (e) => {
		...
		if (touchMapRef.current.size <= 1) {
			...
			if (isPanButton || panByShift) {
				const mode = isSpacePressed.current
					? "shiftPanning"
					: "panning";
	...
	useEffect(() => {
		...
		const handleResizeMove = (e: CustomEvent) => {
			...
			if (
				...
				const scaleX =
					actualWidth > 0 ? newActualWidth / actualWidth : 1;
				const scaleY =
					actualHeight > 0 ? newActualHeight / actualHeight : 1;
				...
				if (actualWidth === 0) {
					...
					newStartX =
						newActualX + (shape.startX - actualMinX) * scaleX;
					newStartY =
						newActualY + (shape.startY - actualMinY) * scaleY;
